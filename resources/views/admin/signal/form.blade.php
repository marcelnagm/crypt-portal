...
<div class="form-group {{ $errors->has('target_1') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="target_1_p" type="number" id="target_1" value="{{ isset($signal->target_1_p) ? $signal->target_1_p : ''}}" >%
...
<div class="form-group {{ $errors->has('target_2') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="target_2_p" type="number" id="target_2" value="{{ isset($signal->target_2_p) ? $signal->target_2_p : ''}}" >%
    {!! $errors->first('target', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('target_3') ? 'has-error' : ''}}">
    <label for="target_3" class="control-label">{{ 'Target 3' }}</label>
    <input class="form-control" name="target_3_p" type="number" id="target_3" value="{{ isset($signal->target_3_p) ? $signal->target_3_p : ''}}" >%
    {!! $errors->first('target', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('stop') ? 'has-error' : ''}}">
    <label for="stop" class="control-label">{{ 'Stop' }}</label>
    <input class="form-control" name="stop_p" type="number" id="stop" value="{{ isset($signal->stop_p) ? $signal->stop_p : ''}}" >%
