...
<div class="form-group {{ $errors->has('target_2') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="target_2_p" type="string" id="target_2" value="{{ isset($signal->target_2_p) ? $signal->target_2_p : ''}}" placeholder="%"
    @if($conf->target_profile >=2)
    required
    @endif
           >
...
<div class="form-group {{ $errors->has('target_3') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="target_3_p" type="string" id="target_3" value="{{ isset($signal->target_3_p) ? $signal->target_3_p : ''}}" placeholder="%"           
    @if($conf->target_profile >=3)
    required
    @endif
           >
